...
<div class="container">
	...
	<div class="box tight">
		<div class="row-fluid">
			<div class="span4"><strong>Drug/Generic</strong></div>
			<div class="span8"><strong>Instruction/Dosage</strong></div>
		</div>
	</div>
		...
		<?php foreach ($pp->getData() as $presc) { //$presc=new PrescriptionData(); ?>
			<?php if ($presc->getId() == $_GET['single']) { ?>
				<div class="box tight">
					<div class="row-fluid">
						<div
							class="span4"><?= is_null($presc->getDrug()) ? $presc->getGeneric()->getName() : $presc->getDrug()->getGeneric()->getName() . ': ' . $presc->getDrug()->getName() ?></div>
						<div class="span8">
							<?php if ($presc->getFilledBy()) { ?>
								<div class="pull-right fadedText pad-right">Dispensed By <?= $presc->getFilledBy()->getFullname() ?></div><?php } ?>
						...
						Quantity Dispensed: <?= $presc->getQuantity() ?> <?= pluralize(ucwords($presc->getGeneric()->getForm()), $presc->getQuantity()) ?>
							...
							<?php } ?>
							<?php if (!is_blank($presc->getComment())) { ?>
								<div class="fadedText pull-right pad-right"><?= $presc->getComment() ?></div><?php } ?>
						</div>
					</div>
				</div>
			<?php } ?>
