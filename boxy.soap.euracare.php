...
$signAndClose = false; //Clinic::$editStyleByAdd;
...
if ($_POST) {
	...
	if (!empty($_POST['family_social_hx'])) {
		$vNote = (new VisitNotes())->setPatient($patObj)->setNoteType('fm_hx')->setDescription($_POST['family_social_hx'])->setDateOfEntry(date('Y-m-d H:i:s'))->setNotedBy($this_user)->setEncounter($encounter);
		...
		foreach ($cases as $i => $case) {
			if (!is_blank($case)) {
				...
				if ((new PatientDiagnosisDAO())->add($diagnosis, $pdo) == null) {
					...
					$summaryNote = trim(ucwords($sevs[$i]) . ' ' ) . $Diagnosis->getName() . ' [' . $Diagnosis->getCode() . ']' . ' (' . ucwords($states[$i]) . ') <span class=fadedText>'.$comments[$i].'</span>';
	...
	if (!empty($_POST['prescription_plan'])) {
		...
		foreach ($notes as $note){
			$vNote = (new VisitNotes())->setPatient($patObj)->setNoteType('plan')->setDescription($note)->setDateOfEntry(date('Y-m-d H:i:s'))->setEncounter($encounter)->setNotedBy($this_user);
			if (!$VNDAO->addNote($vNote, $pdo)) {
				$pdo->rollBack();
				...
				if (!is_blank($_REQUEST['prescription']) && is_blank($reg->pharmacy_id)) {
					...
					foreach ($reg->regimens as $i => $pre) {
						...
						if (!is_blank($pre->body_part)) {
							$pd->setBodyPart((new BodyPartDAO())->get($pre->body_part));
						}
...
<section style="width: 1050px;">
		...
		function ended(s) {
			...
			if (data[0] === "success") {
				Boxy.info(data[1], function () {
					<?php if($signAndClose){?>
					$.post('/api/sign_encounter.php', {encounter_id:<?=$_GET['encounter_id']?>}, function (data) {
	...
	<form name="soapForm" method="post" id="soapForm" action="<?= $_SERVER['REQUEST_URI'] ?>"
		...
		<fieldset>
			...
			<?php if ($clinic_flag === "General") { ?>
				<label>
					Presenting Complaints:
					<textarea name="subjective_note" placeholder="type here ..."><u>Presenting Complaints:</u>
						<br><br><br><br>
						<u>History of complaints:</u>
						<br><br><br><br>
					</textarea>
   ...
   ">
		...
		<fieldset>
			<legend>Family/Social History</legend>
			<label>Family:<textarea name="family_social_hx" rows="2"></textarea></label>
		</fieldset>
		...
		<fieldset>
            ...
            <span>
				...
				<div class="span5 overscrollLabDiv" style="height:300px;min-height:300px">
					<div class="pull-left fadedText">Existing Diagnoses</div>
   ...
   ">
		...
		<fieldset>
			...
			<label>
				<select id="scan_request_ids" multiple="multiple" name="scan_request_ids[]" data-placeholder="select a scan" style="width:99% !important; max-width:99% !important;">
		...
		<fieldset>
			...
			<label>Plan:
				<textarea name="plan_note" placeholder="type here ...">
					<u>Treatment Plan:</u>
						<br><br><br><br></textarea>
			</label>
			<div class="menu-head">Add Medication
				<span class="pull-right">
          <label><input type="checkbox" name="refillable" id="refillable"> Refillable</label>
        </span>
			</div>
			<?php
			require_once($_SERVER['DOCUMENT_ROOT'] . '/classes/DAOs/ServiceCenterDAO.php');
			...
			<div class="row-fluid">
				...
				<label class="span2">Duration <!--<span style="font-size: 90%; font-style: italic; color: #666">--></span>
	...
	<script type="text/javascript">
		...
		$(document).ready(function () {
			...
			$('[name="family_social_hx"]').summernote(SUMMERNOTE_CONFIG);
			setTimeout(function () {
			...
			$("#scan_request_ids").select2({}).change(function (evt) {
				var pid = $('.boxy-content [name="pid"]').val();
				if (evt.added !== undefined) {
					showInsuranceNotice(pid, evt);
					if ($('#scan_request_ids').select2("data")) {
						total = $('#scan_request_ids').select2("data").basePrice;
			...
			$('#load_forms_btn').click(function(e){
				if(!e.handled){
					$('#pre_enc_forms').empty();
					var container = $('<div/>');
					$(document).trigger('ajaxSend');
					var ids = $('[name="pre_enc_forms[]"]').val();
					if(ids !== null){
						$.get('/pages/pm/sforms/sform.view.mini.php?id='+ids).then(function(data){
							container.html(data);
							container.appendTo($('#pre_enc_forms'));
							$(document).trigger('ajaxStop');
						});
					}
			...
			$('input[name="pre_enc_forms[]"]').live('change',function(e){
				if(!e.handled){
					_.each($('input[name="pre_enc_forms[]"]:checked'), function(element){
	...
	<script type="text/javascript">
		...
		function preparePrescription() {
			...
			for (var i = 0; i < prescription.regimens.length; i++) {
				...
				strPrescriptions.push(prescription.regimens[i].dose + ' ' + ((drug !== null) ? prescription.regimens[i].drug.name : gen.name) + ' ' + prescription.regimens[i].freqno + ' x ' + prescription.regimens[i].freqtype.id + ' for ' + prescription.regimens[i].duration + ' day(s)');
			}
