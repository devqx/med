...
class OphItemsRequestDAO
{
	...
	function get($id, $pdo = null)
	{
		if (is_null($id))
			return null;
		$itRequest = new OphItemsRequest();
		try {
			$pdo = $pdo == null ? $this->conn->getPDO() : $pdo;
			$sql = "SELECT * FROM ophthalmology_items_request WHERE id = " . $id;
			...
			if ($row = $stmt->fetch(PDO::FETCH_NAMED, PDO::FETCH_ORI_NEXT)) {
				...
				$itRequest->setItems((new OphItemsRequestDataDAO())->getRequestItems($row['id'], $pdo));
	...
	function all($status = null, $page = 0, $pageSize = 10, $patientId = null, $opCentre = null, $pdo = null)
	{
		...
		$sql = "SELECT * FROM ophthalmology_items_request $filter$filter2$centre";
	...
	function add($request, $pdo = null)
	{
		...
		try {
			...
			$sql = "INSERT INTO ophthalmology_items_request (patient_id, requested_by, time_entered, amount, service_centre_id) VALUES ($patient_id, $requested_by, NOW(), $amount, $service_centre_id)";
			$stmt = $pdo->prepare($sql, array(PDO::ATTR_CURSOR => PDO::CURSOR_SCROLL));
			$stmt->execute();
			$newItems = [];
			if ($stmt->rowCount() == 1) {
				...
				foreach ($request->getItems() as $item) {
					...
					$newItems[] = (new OphItemsRequestDataDAO())->add($item, $pdo);
					$bil = new Bill();
					...
					$bil->setTransactionDate( $request->getRequestTime() );
	...
	function updateStatus($request, $pdo = null)
	{
		try {
			$pdo = $pdo == null ? $this->conn->getPDO() : $pdo;
			$sql = "UPDATE ophthalmology_items_request SET status = '" . $request->getStatus() . "' WHERE id=" . $request->getId();
			$stmt = $pdo->prepare($sql, array(PDO::ATTR_CURSOR => PDO::CURSOR_SCROLL));
			$stmt->execute();
			if ($stmt->rowCount() == 1) {
				return $request;
