...
//        $item = new PhysiotherapyItem();
        try {
            ...
            $bCode = "IP" . generateBillableItemCode('physiotherapy_item', $pdo);
            $item->setCode($bCode);
            $sql = "INSERT INTO physiotherapy_item (billing_code, `name`)  VALUES ('{$item->getCode()}', '" . escape($item->getName()) . "')";
            ...
            if ($stmt->rowCount() == 1) {
                ...
                $insureBI->setItemGroupCategory((new BillSourceDAO())->findSourceById(20, $pdo));
    ...
    function get($lid, $pdo = NULL)
    {
        ...
        try {
            $pdo = $pdo == NULL ? $this->conn->getPDO() : $pdo;
            $sql = "SELECT * FROM physiotherapy_item WHERE id=" . $lid;
            $stmt = $pdo->prepare($sql, array(PDO::ATTR_CURSOR => PDO::CURSOR_SCROLL));
            $stmt->execute();
            if ($row = $stmt->fetch(PDO::FETCH_NAMED, PDO::FETCH_ORI_NEXT)) {
                return (new PhysiotherapyItem())
                    ->setId($row['id'])
    ...
    function getByCode($iCode, $pdo = NULL)
    {
        ...
        try {
            $pdo = $pdo == NULL ? $this->conn->getPDO() : $pdo;
            $sql = "SELECT * FROM physiotherapy_item WHERE billing_code='" . $iCode . "'";
            $stmt = $pdo->prepare($sql, array(PDO::ATTR_CURSOR => PDO::CURSOR_SCROLL));
            $stmt->execute();
            if ($row = $stmt->fetch(PDO::FETCH_NAMED, PDO::FETCH_ORI_NEXT)) {
                // return (object)$row;
    ...
    function update($item, $pdo = NULL){
        try {
            $pdo = $pdo == NULL ? $this->conn->getPDO() : $pdo;
            $pdo->beginTransaction();
            $sql = "UPDATE physiotherapy_item SET `name` = '".escape($item->getName())."' WHERE id = ". $item->getId();
            $stmt = $pdo->prepare($sql, array(PDO::ATTR_CURSOR => PDO::CURSOR_SCROLL));
            $stmt->execute();
    ...
    function all($pdo = NULL)
    {
        ...
        try {
            $pdo = $pdo == NULL ? $this->conn->getPDO() : $pdo;
            $sql = "SELECT * FROM physiotherapy_item";
            $stmt = $pdo->prepare($sql, array(PDO::ATTR_CURSOR => PDO::CURSOR_SCROLL));
            $stmt->execute();
    ...
    function findItems($search, $pdo = NULL)
    {
        ...
        try {
            $pdo = $pdo == NULL ? $this->conn->getPDO() : $pdo;
            $sql = "SELECT * FROM physiotherapy_item WHERE `name` LIKE '%$search%'";
            $stmt = $pdo->prepare($sql, array(PDO::ATTR_CURSOR => PDO::CURSOR_SCROLL));
            $stmt->execute();
            while ($row = $stmt->fetch(PDO::FETCH_NAMED, PDO::FETCH_ORI_NEXT)) {
