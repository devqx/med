...
$pageSize = 10;
$data = (new PatientItemRequestDAO())->getAllItems($page, $pageSize, $center,  TRUE, $patient, null,  $statusFilter="filled");
...
<div id="filled_request"><a href="javascript:;" onclick="aTab(2)" data-href="?filled">Refresh List</a></div>
<?= (($totalSearch > 0) ? '' : '<div class="notify-bar">There are no filled requests</div>') ?>
			...
			if ($p->getPatient() != null) { ?>
				<tr>
					<td nowrap class="pres_details"
					    data-href="/consumableItems/boxy.fillbatch.php?pCode=<?= $p->getCode() ?>&pid=<?= $p->getPatient()->getId() ?>"><?= date("d M, Y", strtotime($p->getRequestDate())) ?></td>
					<td class="pres_details"
					    data-href="/consumableItems/boxy.fillbatch.php?pCode=<?= $p->getCode() ?>&pid=<?= $p->getPatient()->getId() ?>"><?= $p->getCode() ?></td>
					<td class="pres_details"
...
<?php } ?>
<script>
	$('select[name="service_center_id"]').select2({width: '100%', allowClear: true}).change(function (e) {
		if(!e.handled){
			goto(0);
			e.handled = true;
		}
	...
	function goto(page) {
		$('#prescription_container').load('filled_request.php?page=' + page + '&service_center_id=' + $('select[name="service_center_id"]').val()+'&patient_id='+$('[name="patient_id"]').val());
	}
</script>