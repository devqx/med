...
$oph_centre = (isset($_REQUEST['ophthalmology_centre_id']) && !is_blank($_REQUEST['ophthalmology_centre_id'])) ? $_REQUEST['ophthalmology_centre_id'] : null;
$data = (new OphItemsRequestDAO())->all($_REQUEST['status'], $page, $pageSize, $patientId, $oph_centre);
$result = $data->data;
...
<div class="dataTables_wrapper" id="resultPage">
	...
	<?php } else { ?>
		<table class="table table-striped">
			...
			<tbody>
			<?php foreach ($result as $r) {//$r=new OphItemsRequest;?>
				<tr>
					...
					<td>
						...
						<?php if (in_array($r->getStatus(),["Open", "Received"])) { ?>| <a href="javascript:" class="cancelRequest" data-id="<?= $r->getId() ?>">Cancel</a><?php } ?>
					</td>
				</tr>
			<?php } ?>
			</tbody>
