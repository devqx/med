...
require_once $_SERVER['DOCUMENT_ROOT'] . '/classes/DAOs/OphItemsRequestDataDAO.php';
require_once $_SERVER['DOCUMENT_ROOT'] . '/functions/utils.php';
require_once $_SERVER['DOCUMENT_ROOT'] . '/classes/OphItemsRequestData.php';
require_once $_SERVER['DOCUMENT_ROOT'] . '/classes/OphItemsRequest.php';
...
$centres = (new ServiceCenterDAO())->all('Ophthalmology');
@session_start();
if($_POST){
    if(is_blank($_POST['service_centre_id'])){exit("error:Select service centre");}
    if(is_blank($_POST['patient_id'])){exit("error:Select Patient");}
    if(empty(array_filter(explode(",",$_POST['items']))) ){exit("error:Select Request Items");}
...
//    foreach ($itemsList as $item_) {
    foreach ($items_ as $item_) {
        $item = (new OphthalmologyItem())
            ->setId($item_->id)
            ->setBasePrice($item_->basePrice)
            ->setCode($item_->code)
            ->setName($item_->name);
        $items[] = (new OphItemsRequestData())->setItem($item);
    }

    $request = (new OphItemsRequest())
        ->setAmount(parseNumber($_POST['amount']))
        ->setServiceCentre( new ServiceCenter($_POST['service_centre_id']) )
        ->setPatient( new PatientDemograph($_POST['patient_id']) )
        ->setRequester( new StaffDirectory($_SESSION['staffID']) )
        ->setItems($items);
    if((new OphItemsRequestDAO())->add($request) !== NULL){
...
<section class="document" <?= isset($_REQUEST['patient_id'])? ' style="width: 650px"':'' ?>>
    ...
    <form method="post" action="<?= $_SERVER['REQUEST_URI']?>" onsubmit="return AIM.submit(this, {onStart: null, onComplete: finishRequest})">
        <label> Business Unit/Service Center <select name="service_centre_id" placeholder="Select a Receiving center">
                <option></option>
        ...
        <div class="btn-block">
            ...
            <?php if(isset($_REQUEST['patient_id'])){?><button class="btn-link" type="reset" onclick="Boxy.get(this).hideAndUnload();">Cancel</button><?php }?>
        </div>
    </form>
...
<script type="text/javascript">
    $(document).ready(function(){
        ...
        $('#items').select2({
            placeholder: "Search and select Optometry item",
            minimumInputLength: 3,
            width: '100%',
            multiple: true,
            allowClear: true,
            ajax: {
                url: "/api/get_ophthalmology_items.php",
                dataType: 'json',
                data: function (term, page) {
                    return {
                        search: term
            ...
            if(evt.added != undefined){
                vex.dialog.prompt({
                    message: 'Press Cancel if you would use the default price of <span class="naira"></span>'+evt.added.basePrice,
                    placeholder: 'Enter new Price',
                    value: evt.added.basePrice,
                    overlayClosesOnClick: false,
                    callback: function(value){
                        if(value !== false && value !== '' ) {evt.added.basePrice = value;}
                        $this.trigger('change');
                    }
                });
    ...
    var finishRequest = function(s){
        ...
        if(data[0] === "error"){
            ...
            Boxy.info(data[1]);
            <?php if(isset($_REQUEST['patient_id'])){?>showTabs(17,2);Boxy.get($(".close")).hideAndUnload();<?php }else {?>
