...
$inPatients = (new InPatientDAO())->getMyInPatient($_SESSION['staffID'], true, $block, $ward, $page, $pageSize, $h_id);
...
<div id="admission_container" class="dataTables_wrapper">
	...
	<table class="table table-hover table-striped">
		...
		<tbody>
		<?php foreach ($inPatients->data as $ip) {//$ip = new InPatient();
			?>
			<tr>
				<td nowrap>
                    ...
                    <?php if(  ! empty($inp_health_state['risk_to_fall'] ) && $inp_health_state['risk_to_fall'] == 1){?>
                        <i class="fa fa-exclamation-triangle notif-icons fa-2x" title=" <?php echo $ip->patientName;?>'s Risk To Fall "  style="color:red;cursor"></i>
                    <?php }?>
				...
				<td><?= ($ip->bed_id != null ? $ip->bedName . " (" . $ip->roomName . ")" : "") ?>
					<?php if ($ip->bed_id == null) { ?><a href="javascript:;" onclick="assignBed('<?= $ip->id ?>', 3)" class="btn btn-small">Assign Bed</a><?php } ?></td>
			</tr>
		<?php } ?>
		</tbody>
	...
	<div class="resultsPagerOpenMy no-footer dataTables_paginate">
...
<script>
	$(document).ready(function () {
        ...
        $('select[name="block_id"]').select2({
			...
			if (!e.handled) {
				$.post('/admissions/homeTabs/myPatients.php', {'page': 0, 'block_id': $('select[name="block_id"]').val(), 'ward_id': $('select[name="ward_id"]').val(), 'patient_id': $('input[name="patient_id"]').val(), 'health_state_id': $('input[name="health_state"]').val()}, function (s) {
					$('#admission_container').html(s);
					loadPatient();
				});
				e.handled = true;
		...
		$(document).on('click', '.resultsPagerOpenMy.dataTables_paginate a.paginate_button', function (e) {
			if (!$(this).hasClass("disabled") && !e.handled) {
				var page = $(this).data("page");
				$.post('/admissions/homeTabs/myPatients.php', {'page': page, 'block_id': $('select[name="block_id"]').val(), 'ward_id': $('select[name="ward_id"]').val(), 'patient_id': $('input[name="patient_id"]').val(), 'health_state_id': $('input[name="health_state"]').val()}, function (s) {
					$('#admission_container').html(s);
					loadPatient();
				});
				e.handled = true;
