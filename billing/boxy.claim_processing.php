...
<section style="width: 900px" id="row-fluid">
	<div class="row-fluid">
		<?php if ((!in_array($claim->getState(),  ['confirmed', 'represented', 'rendered']))) { ?>
	...
	<div class="row-fluid" style="margin-top: -20px;">
		<?php if ((!in_array($claim->getState(),  ['confirmed', 'represented', 'rendered', 'validated']))) { ?>
		<button class="drop-btn mini" href="javascript:;" onclick="Validate()"> Validate</button>
		<?php } ?>
		<?php if ((!in_array($claim->getState(),  ['confirmed', 'represented', 'rendered','draft']))) { ?>
		<button class="drop-btn mini"  href="javascript:;" onclick="confirm()"> Confirm</button>
		<?php } ?>
	</div>
	<form method="post" action="<?= $_SERVER['REQUEST_URI'] ?>" onsubmit="return AIM.submit(this, {onStart: s, onComplete: d2})" style="margin-top: 20px;">
		<?php if ((!in_array($claim->getState(),  ['confirmed', 'represented', 'rendered']))) { ?>
		<label>
			Select Encounter
		...
		<table id="claim_lines" class="table table-striped">
			...
			<tbody>
			<?php if (count($claimLine) == 0 || (in_array($claim->getState(),  ['confirmed', 'represented', 'rendered']))) { ?>
				<tr>
					<td colspan="5">
...
<script type="text/javascript">
	...
	function Validate() {
		$.post ('/billing/validate.claim.php', {claim_id:<?= $_GET['id'] ?>, type:'validate' },
			function(s){
				var data = s.split(":");
				if(data[0] === 'success'){
					Boxy.info(data[1], function () {
						setTimeout(function () {
							...
							setTimeout(function () {
								Boxy.load('/billing/boxy.claim_processing.php?id=<?=$_GET['id']?>');
							}, 100);
						}, 100);
					});
				}else{
	...
	function confirm() {
		$.post ('/billing/validate.claim.php', {claim_id:<?= $_GET['id'] ?>, type:'confirm' },
			function(s){
				var data = s.split(":");
				if(data[0] === 'success'){
					Boxy.info(data[1], function () {
