...
$_GET['items_'] = isset($_SESSION['checked_items_all']) ? implode(',', $items) : '';

$data = (new BillDAO())->getBillsToClaim($_GET['items'], True);
...
<section id="docu" style="width: 650px;">
	...
	<form id="startPrivateClaimForm" method="post" action="<?= $_SERVER['REQUEST_URI'] ?>" onsubmit="return AIM.submit(this, {onComplete: processing_bills, onStart: start_process})">
	...
	<script type="text/javascript">
		function start_process() {
			$('#startPrivateClaimForm').parent().parent().block({
		...
		function processing_bills(s) {
			var area = $('#startPrivateClaimForm').parent();
			area.parent().unblock();
			try {
