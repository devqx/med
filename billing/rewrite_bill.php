...
if ($_POST) {
	//exit("error:Sorry, this function has been deprecated");
	...
	$oldId = $billCopy->getId();
	$category = getItem($billCopy->getItemCode(), $pdo);
	...
	foreach ($Parts as $part) {
		if ($part->transaction_type == 'write-off') {
			...
			//$bil3->setCostCentre($bill->getCostCentre());
			...
			$bil3->setParent($bill);
			$bil3->setReceiver(new StaffDirectory($_SESSION['staffID']));
			...
			//}
			break;
		}else if($part->transaction_type == 'credit'){
			 $writeOffPart = $part;
		}
	}
	...
	$bil->setAmount((float)(0 - ($bill->getAmount() +  ($writeOffPart ? ($writeOffPart->amount * -1) : 0) )));
	$bil->setDiscounted(null);
	$bil->setDiscountedBy(null);
	$bil->setClinic(new Clinic(1));
	$bil->setBilledTo($bill->getBilledTo());
	//$bil->setCostCentre($bil->getCostCentre());
	...
	//fixme: really DO NOT add this line
	...
	if ($bill->getTransactionType() == "transfer-credit") {
		...
		$bill1 = $bill->setAmount(0 - abs($amount))->setReceiver(new StaffDirectory($_SESSION['staffID']))->setTransactionType("transfer-debit")->setItem(null)->setItemCode($bill->getItemCode())->setBilledTo($bill->getBilledTo())->setCostCentre($bill->getCostCentre())->setTransactionDate(date('Y-m-d H:i:s'))->add($bill->getQuantity(), $ipId, $pdo);
		$bill2 = $bill->setAmount(abs($amount))->setReceiver(new StaffDirectory($_SESSION['staffID']))->setTransactionType("transfer-credit")->setItem(getItem($bill->getItemCode(), $pdo))->setTransactionDate(date('Y-m-d H:i:s'))->setCostCentre($bill->getCostCentre())->setTransferred(false)->setBilledTo(new InsuranceScheme(1))->add($bill->getQuantity(), $ipId, $pdo);
		
	} else if ($bill->getTransactionType() == 'credit') {
		$bill1 = $bill->setAmount(0 - abs($amount))->setReceiver(new StaffDirectory($_SESSION['staffID']))->setTransactionType("debit")->setItem(null)->setItemCode($bill->getItemCode())->setBilledTo($bill->getBilledTo())->setTransactionDate(date('Y-m-d H:i:s'))->setCostCentre($bill->getCostCentre())->add($bill->getQuantity(), $ipId, $pdo);
		$bill2 = $bill->setAmount(abs($amount))->setReceiver(new StaffDirectory($_SESSION['staffID']))->setTransactionType("credit")->setItem(getItem($bill->getItemCode(), $pdo))->setTransactionDate(date('Y-m-d H:i:s'))->setTransferred(false)->setCostCentre($bill->getCostCentre())->setBilledTo(new InsuranceScheme(1))->add($bill->getQuantity(), $ipId, $pdo);
	}
	//we have introduced the item codes into the debit parts
	...
	$newId = $bill2->getId();
	$oldBill = (int)$billCopy->getId();
	
	$replace = (new BillDAO())->replaceBill($oldBill, $newId, $category, $pdo);//continue here and the other
	if (in_array($replace, [true, null])) {
		$pdo->commit();
		exit("success:Success");
	} else {
		$pdo->rollBack();
		exit('error:Failed to reverse service charge');
	}
}
?>
