...
foreach ($claimsReport as $lines) {
	...
	$diagnosis = (new PatientDiagnosisDAO())->one($lines->patient_id,  $type=null, $active=true, $severity=null, $page, $pageSize,null, $lines->encounter_id);
	//    $visitsNotes = (new Patient)
...
<form id="filterForm" method="post">
	<div id="filter2">
		...
		<div class="clearfix filter row-fluid">
			...
			<input type="text" name="provider" value="<?= $_GET['provider_id'] ?>">
			<input type="text" name="insurance_scheme_id" value="<?= $_GET['sid'] ?>">
				...
				<?php if (isset($return) && sizeof($return) > 0) {
					foreach ($return as $report) {?>
						<tr>
							...
							<td nowrap><ul>
									<?php foreach ($report->Diagnosis as $diag) { ?>
										<li>
											<?= strtoupper($diag->diagnosisType)?> (<?=trim($diag->code)?>): <?=$diag->case?></li>
									<?php } ?>
								</ul>
							</td>
