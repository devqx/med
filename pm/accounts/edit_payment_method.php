...
$method = (new PaymentMethodDAO())->get($_GET['id'], TRUE);

if ($_POST) {
	//the url still contains the $_GET object in the form action url
	if(is_blank($_POST['name'])){exit("error:Name can't be blank");}
	if(is_blank($_POST['type'])){exit("error:Type of method is required");}

	$method = (new PaymentMethodDAO())->get($_GET['id'], TRUE)->setName($_POST['name'])->setType($_POST['type'])->setLedgerId($_POST['ledger_id'])->update();
	if ($method !== null) {
...
<section>
	<form action="<?= $_SERVER['REQUEST_URI'] ?>" method="post" onsubmit="return AIM.submit(this, {onComplete:___posted})">
		<label>Name <input type="text" name="name" value="<?= $method->getName() ?>"></label>
		<label>Type <select name="type"><?php foreach ($pay_Types as $types) { ?>
					<option value="<?= $types ?>" <?= ($method->getType() == $types) ? ' selected="selected"' : '' ?>><?= ucfirst($types) ?></option>
				<?php } ?>
			</select></label>
		<label>ERP Ledger <input type="text" name="ledger_id" value="<?= $method->getLedgerId() ?>"></label>
		<div class="btn-block">
