...
if($_POST){
    ...
    foreach ($_POST['batch'] as $itemId => $batchId) {
        ...
        $result[] = (new PhysiotherapyItemBatchDAO())->depleteStock($batch, 1, $pdo) ;
    }
...
<section style="width: 500px">
    ...
    <form method="post" action="<?= $_SERVER['REQUEST_URI']?>" onsubmit="return AIM.submit(this, {onStart: null, onComplete: received})">
                ...
                <tr>
                    ...
                    <td>
                        <label><select name="batch[<?= $i->getItem()->getId()?>]"><?php
                                ...
                                foreach ($batches as $b) {/*$b=new PhysiotherapyItemBatch();*/if($b->getQuantity() > 0){?>
        ...
        <?=($selfOwe > 0)? ' <div class="warning-bar">Patient has an outstanding balance of <span class="naira"></span>'.$selfOwe.'</div><p></p>':'' ?>
        <button class="btn" type="submit" <?=($selfOwe - $creditLimit > 0)? ' disabled="disabled"':'' ?>>Deliver</button>
