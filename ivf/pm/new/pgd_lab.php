...
if($_POST){
    ...
    $item = (new GeneticLabDAO())->add( (new GeneticLab())
        ->setName($_POST['name'])
        ->setPrintLayout($_POST['print_layout'])
        ->setQualityControls($_POST['qc_items'])
        ->setTemplate( new GeneticTemplate($_POST['template_id']))
        ->setBasePrice($_POST['price'])
    );
    if(!is_null($item)){
...
<section>
    <form method="post" action="<?= $_SERVER['REQUEST_URI']?>" onsubmit="return AIM.submit(this, {onComplete: finish})">
        ...
        <label>Default Charge <input type="number" value="0" name="price" required> </label>
        ...
        <div class="row-fluid">
            <label class="span12"><input type="radio" name="print_layout" value="portrait"> Portrait </label>
        </div>
        <div class="row-fluid">
            <label class="span12"><input type="radio" name="print_layout" value="landscape"> Landscape </label>
        </div>
        ...
        <button type="submit" class="btn">Add GeneticLab</button>
