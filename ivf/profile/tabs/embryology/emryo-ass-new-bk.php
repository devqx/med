...
if ($_POST) {
	...
	foreach ($_POST['embryo_no'] as $i => $item) {
		...
		$data[] = (new EmbryoAssessmentData())->setEmbryoNumber($_POST['embryo_no'][$i])->setCellNumber($_POST['cell_no'][$i])->setQuality($_POST['grade_quality'][$i])->setMorula($_POST['morula'][$i])->setBlastocyst($_POST['blastocyst'][$i]);//->setState($_POST['embryo_state'][$i]);
	}
		...
		<div class="paper-card">
			...
			<div class="card-content">
				<div class="row-fluid">
					...
					<label class="span4 hidden">Cell # <input data-decimals="0" type="number" min="1" max="17" step="1" name="cell_no[]"> </label>
					<label class="span4">Grade/Quality <input type="text" name="grade_quality[]"> </label>
				</div>
				<div class="row-fluid">
					<label class="span4">Morula %<input data-decimals="0" type="number" name="morula[]"> </label>
					<label class="span4">Blastocyst <input data-decimals="0" type="number" name="blastocyst[]"> </label>
					<label class="span4 hidden">Status
						<select name="embryo_state[]" class="hidden">
