...
foreach($ids as $id){
	...
	<section  class="form">
		...
		<h5 class="fadedText">Category: <?= $form->getCategory()->getName() ?></h5>
		<div class="clearfix ">
		...
		<?php foreach ($form->getQuestions() as $j => $question) {//$question=new SFormQuestion();?>
			<div class="data">
				...
				<?php if(in_array($question->getType(), ['number', 'text', 'date'])){?>
					<div class="row-fluid">
						<label class="span12">
							<input type="<?=$question->getType()?>" name="question[<?=$question->getId()?>]">
						</label>
					</div>
				<?php }?><?php if( in_array($question->getType(), ['longtext'])){?>
					<div class="row-fluid">
						<label class="span12">
							<textarea name="question[<?=$question->getId()?>]"></textarea>
						</label>
					</div>
				<?php }?>
				<?php foreach ($question->getOptions() as $i => $option) {//$option=new SFormOption();?>
					<div class="data-options ">
						<div class="row-fluid">
							<label class="span11"><input name="question[<?=$question->getId()?>][]" type="<?= $question->getType() ?>" value="<?= $option->getId() ?>"> <?= $option->getText() ?></label>
						</div>
					</div>
				<?php } ?>
			</div>
