...
if($_POST){
    ...
    $batch = new PhysiotherapyItemBatch();

    if(!is_blank($_POST['name'])){
        $batch->setName($_POST['name']);
        $batch->setItem(new PhysiotherapyItem($_POST['item_id']));
    }else {
    ...
    $new = (new PhysiotherapyItemBatchDAO())->add($batch);
    if($new !== NULL){
        exit("ok");
    }
...
<section style="width: 650px">
    <form method="post" action="<?= $_SERVER['REQUEST_URI']?>" onsubmit="return AIM.submit(this, {onComplete: saved})">
        <label>Physiotherapy Center <select name="service_centre_id">
