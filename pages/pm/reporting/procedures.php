...
<form id="filterForm" class="document" method="post" action="/pm/reporting/procedures.php">
...
<script type="text/javascript">
    ...
    $(document).ready(function () {
        ...
        $('#exportIT').on('click', function(e){
            if(!e.handled) {
                window.open('/excel.php?dataSource=procedures&filename=Procedure_Report&from=<?=(isset($_REQUEST['from']))?$_REQUEST['from']:''?>&to=<?=(isset($_REQUEST['to']))?$_REQUEST['to']:''?>&category_id=<?=(isset($_REQUEST['category_id']))?$_REQUEST['category_id']:''?>', '_blank');
                e.handled = true;
                e.preventDefault();
            }
        });
