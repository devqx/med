...
$item = (new PhysiotherapyItemDAO())->get($_GET['id']);
$batches = (new PhysiotherapyItemBatchDAO())->getItemBatches( $item );
$protect = new Protect();
...
<section style="width: 700px;">
    ...
    <table class="table table-striped">
        ...
        <?php if(count($batches)>0){ foreach ($batches as $b) {//$b=new PhysiotherapyItemBatch();?>
            <tr>
...
<script type="text/javascript">
    $(document).ready(function () {
        $('#addNewBatch').live('click',function (e) {
            $ID= $(this).data("id");
            if(!e.handled){
                Boxy.load('/pages/pm/physiotherapyItems/batch.new.php?d_id='+$ID, {afterHide:function(){
                    setTimeout(function () {
                        Boxy.get($(".close")).hideAndUnload();
                        setTimeout(function () {
                            Boxy.load('/pages/pm/physioItemBatch.php?id='+$ID);
                        }, 100);
                    }, 100);
                }});
                e.handled = true;
        ...
        $('a.add_Stock').bind('click',function () {
            $ID = $(this).data("id");
            Boxy.load('/pages/pm/physiotherapyItems/batch.add.php?batch='+$ID,{afterHide:function(){
                setTimeout(function () {
                    Boxy.get($(".close")).hideAndUnload();
                    setTimeout(function () {
                        Boxy.load('/pages/pm/physioItemBatch.php?id=<?=$_GET['id']?>');
                    }, 100);
                }, 100);
            }});
        });
        $('a.adjust_Stock').bind('click', function () {
            $ID = $(this).data("id");
            Boxy.load('/pages/pm/physiotherapyItems/batch.adjust.php?batch='+$ID,{afterHide:function(){
                setTimeout(function () {
                    Boxy.get($(".close")).hideAndUnload();
                    setTimeout(function () {
                        Boxy.load('/pages/pm/physioItemBatch.php?id=<?=$_GET['id']?>');
